...
/*   Updated: 2025/05/02 04:29:30 by tdelage          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef t_size				(*t_stream_read_f)(t_ptr buffer, t_size len,
					t_ptr instance);
typedef t_size				(*t_stream_write_f)(t_ptr buffer, t_size len,
					t_ptr instance);
typedef void				(*t_stream_end_f)(t_ptr instance);
typedef t_size				(*t_stream_available_f)(t_ptr instance);

typedef struct s_stream
{
	t_stream_read_f			read;
	t_stream_write_f		write;
	t_stream_end_f			end;
	t_stream_available_f	available;
	t_ptr					instance;
}							t_stream;

t_size						stream_read(t_stream *stream, t_ptr buffer,
								t_size len);
